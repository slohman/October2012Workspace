...
import com.actionbarsherlock.view.MenuInflater;
	...
	public class fltreportlists extends SherlockActivity {
	    ...
	    int names[] = {android.R.id.text1,android.R.id.text2};
	 		final Cursor cu = db.rawQuery("Select _id,blk,Date,Flt ||' '|| frm ||' ' ||  dest AS Fdata from fltlog where Date >= '" + srchDate  +  "'  and land <> 'DHD FLT' order by Date ASC" ,null);
	 		cu.moveToFirst();
	 		
	 		
	 		 final Cursor cu1 = db.rawQuery("Select _id,land,date from fltlog where Date >= '" + srchDate  +  "'  and land <> 'DHD FLT' order by Date ASC" ,null);
	 		...
	 		//above gets who did the landing and counts the total up for the time period
	 			...
	 			cu.moveToNext();
	 		     ...
	 		     if ((totalBlkMin < 10) && (totalBlkHours < 10)){
	 		 	    ttl.setText(ttl.getText().toString()+ " Total Blk Time & Landgins: "+  " " +  " " + "0" + sTBH + ":0" + sTBM + "  "+"C:"+ Clandings + "  "+"F:"+ Flandings);
	 		 	      }
	 		 		      if ((totalBlkMin < 10) && (totalBlkHours > 9)){
	 		 		    	ttl.setText(ttl.getText().toString() + " Total Blk Time & Lndings: "+  " "  + sTBH + ":0" + sTBM + "  "+"C:"+ Clandings + "  "+"F:"+ Flandings);  
	 		 		  }
	 		 		      if  ((totalBlkMin > 10) && (totalBlkHours > 9)){
